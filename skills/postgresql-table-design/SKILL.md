...
description: >-
  Design e revisão de schema PostgreSQL: tipos, indexes, constraints, partitioning,
  JSONB e gotchas. Invocar em Specify/Plan/Execute quando o To Be, design.md, tasks
  ou migrations tocarem tabelas Postgres. Não substitui as fases SDD.
---
...
Aplicar práticas sólidas de schema PostgreSQL ao desenhar ou revisar modelo de dados,
migrations e DDL — sem inventar convenções genéricas de “SQL qualquer”.

Tipo: **referência técnica** — flexível na aplicação ao domínio; **rígida** nos gotchas
e nas proibições de tipos listados abaixo (corpo em inglês).

Não substitui Specify, Plan nem Execute.

## Quando invocar

| Momento | Quem dispara |
|---------|--------------|
| To Be / `design.md` com modelo de dados Postgres | `/sdd-03-specify` |
| Tasks de schema, migration ou indexes Postgres | `/sdd-04-plan` |
| Implementar DDL/migrations/tabelas Postgres | `/sdd-06-execute` |
| Review de schema proposto no chat | auto (description) |

**Não invocar quando:** banco não é PostgreSQL; mudança sem schema (só app/API);
projeto sem Postgres e sem plano de adotá-lo.

## Contexto do projeto

Versão do Postgres, extensões (PostGIS, pgvector, Timescale, …) e convenções de
migration: `AGENTS.md`. Se não estiver documentado e importar para a decisão
(ex.: `uuidv7` vs `gen_random_uuid`, `NULLS NOT DISTINCT`) → **pergunte** ao dev.

## Integração SDD

1. **Specify** — alinhar entidades, tipos e restrições do To Be / `design.md` a estas regras.
2. **Plan** — Steps de migration/DDL devem refletir indexes de FK, tipos e evolução segura.
3. **Execute** — ao escrever SQL/ORM schema, seguir o corpo técnico; não “simplificar” gotchas.

## Red flags

| Racionalização | Realidade |
|----------------|-----------|
| "VARCHAR(n) / SERIAL / TIMESTAMP sem TZ estão ok" | Ver seção *Do not use* abaixo |
| "FK já cria index" | Postgres **não** indexa FK automaticamente |
| "É só uma tabelinha, pulo a skill" | Gotchas aparecem em schemas pequenos |
| "Denormalizo agora pra ficar mais rápido" | Normalizar a 3NF primeiro; denormalizar só com evidência |

## Saída esperada

Decisões de schema (tipos, PK/FK, indexes, constraints, partitioning) alinhadas às
regras abaixo; lacunas de versão/extensão resolvidas via `AGENTS.md` ou pergunta ao dev.
